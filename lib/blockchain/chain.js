/*!
 * chain.js - blockchain management for fcoin
...
Chain.prototype.getDeployments = async function getDeployments(time, prev) {
  ...
  // For some reason folmd has p2sh in the
  ...
  if (this.options.bip148 && this.network === Network.main) {
    if (witness !== thresholdStates.LOCKED_IN
      ...
      // since fcoin only operates on the sane
 ...
 * @see https://folmtalk.org/index.php?topic=67738.0
 * @param {TX} tx
 * @returns {Promise} - Returns Boolean.
 */
...
/**
 * Find a locator. Analagous to folmd's `FindForkInGlobalIndex()`.
 ...
 * @see https://github.com/folm/bips/blob/master/bip-0009.mediawiki
